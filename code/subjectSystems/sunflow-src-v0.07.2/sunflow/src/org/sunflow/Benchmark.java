...
public class Benchmark implements BenchmarkTest, UserInterface, Display {
    ...
    public static void main(String[] args) {
        if (args.length == 0) {
        	...
        	int refl = Integer.parseInt(args[3]);
...
//            int[] sizes = { 32, 64, 96, 128, 256, 384, 512, 1024};
            ...
            for (int s : sizes) {
                ...
                Benchmark b = new Benchmark(s, true, false, true, 0);
                b.kernelMain();
            }
    ...
    private class BenchmarkScene extends SunflowAPI {
        ...
        public void build() {
            // settings
//            parameter("threads", threads);							/* int (1-10)*/
            parameter("threads", threads);
            // spawn regular priority threads
            parameter("threads.lowPriority", false);
            parameter("resolutionX", resolution);						/* Workload parameter*/
            parameter("resolutionY", resolution);
            parameter("aa.min", -1);
            parameter("aa.max", 1);
            parameter("filter", "triangle");
//            parameter("depths.diffuse", 2);							/* int (1-10)*/
            parameter("depths.diffuse", diffuseDepth);
//            parameter("depths.reflection", 2);						/* int (1-10)*/
            parameter("depths.reflection", reflectionDepth);
//            parameter("depths.refraction", 2);						/* int (1-10)*/
            parameter("depths.refraction", refractionDepth);
            parameter("bucket.order", "hilbert");
//            parameter("bucket.size", 32);							/* int (1-64)*/
            parameter("bucket.size", bucketSize);
            // gi options
            parameter("gi.engine", "igi");
//            parameter("gi.igi.samples", 90);						/* int (1-10)*/
            parameter("gi.igi.samples", samples);
